...
show(level);
var tr = $('tr'); 
$(tr[tr.length - (level + 1)]).css('background','#d8d823');
...
$('.btn').click(function(){
	...
	$(tr[tr.length - (level + 1)]).css('background','#d8d823');
	$(tr[tr.length - (level)]).css('color','black');
...
var count = 0;
	while(count < 2) {
		...
		{
			$(inputLabel[index]).css('color', 'rgb(31, 35, 34)');
			count++;
			exp.push(index);
		}
	}
